...
# --- Configuration ---
PROJECT_ID="prod-volto-interviews"
REGION="europe-west3"
...
if [ "$DEPLOY_BACKEND" = true ]; then
  ...
  gcloud run deploy "$BACKEND_SERVICE_NAME" \
    ...
    --set-env-vars="ENVIRONMENT=prod" # In a real scenario, you might restrict this
else
...
# Check for production environment file first, then fall back to regular .env
if [ -f "$PROD_ENV_FILE" ]; then
  echo "📁 Using production environment file: $PROD_ENV_FILE"
